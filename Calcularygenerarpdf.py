...
def parseredgesA(file):
    ...
    for i, linea in enumerate (datos):
        nums = linea.split(", ")
        edgesA.append(arista(i, str(nums[0]), str(nums[1]), str(nums[2]), float(nums[3]), float(nums[4]), float(nums[5]), float(nums[6]), float(nums[7]), float(nums[8])))
    return edgesA
class arista:
    ...
    def getsem(self):
        return self.__sem
class cromosoma:
    def __init__(self, edgesA = []):
        self.__edgesA = list(edgesA)
    def copy(self):
        return cromosoma( self.getedgesA())
    def __del__(self):
        while self.__edgesA:
            arista = self.__edgesA.pop()
        del arista
    def getedgesA(self):
        return self.__edgesA
    def __str__(self):
        s  = "["
        for arista in self.getedgesA() :
            s += " %s," % (str(arista))
        return s
    def costo(self, nd1):
        a = nd1
    ...
    def aptitud( self ):
        apt = 0
        orig = sorted(self.getedgesA(), key=lambda x: x.getid(), reverse=False)
        for a in orig:
            apt += self.costo(a)
        return apt
    ...
    def index( self, nodo ):
        return self.__edgesA.index(nodo)
    def cruzar(self, otro):
        p1, p2 = self.getedgesA(), otro.getedgesA()
...
def cruzarpob( pob, pcruza ):
	...
	return hijos
def generapob(edgesA, pob):
	s     = list()
	perms = permutaciones(edgesA)
	fin   = aleatorio( pob, fact( len(edgesA) ) )
	ini   = fin - pob
...
def crear(edgesA):
    caminos = list()
    for arista in edgesA:
        camino = buscar(arista, edgesA)
        if camino not in caminos:
            caminos.append(camino)
    return caminos
camino = []
tmpcamino = []
def buscar(aristaOriginal, edgesA):
    global camino
    global tmpcamino
    tmpcamino.append(aristaOriginal)
    for arista in edgesA:
        if aristaOriginal.getdest() == arista.getorg():
            buscar(arista, edgesA)
...
padres = []
def buscarM(origen, edgesA):
    for arista in edgesA:
        if arista.getdest() == origen:
            padres.append(arista)
            buscarM(arista.getorg(), edgesA)
    return padres
def main():
    edgesA = parseredgesA("edgesA.txt")
    ...
    for arista in sorted(respuestaFinal.getedgesA(), key=lambda x: x.getid(), reverse=True):
        dot2.node(arista.getorg())
        dot2.edge(arista.getorg(), arista.getdest(), str(arista.gettrf()))
    lista = sorted(respuestaFinal.getedgesA(), key=lambda x: x.getid(), reverse=True)
    padre = None
    while padre is None:
        padre = buscarM(lista[0].getorg(), edgesA)
    if padre is not None:
        if len(padre) > 0:
            for hijo in padre:
                if hijo not in respuestaFinal.getedgesA():
